  ...
  return (
    <Card sx={styles.card}>
      <Grid
        sx={{
          ...styles.innerCard,
          borderLeft: `3px solid ${todoColor(taskData.date)}`,
        }}
      >
        <CardContent sx={styles.cardContainer}>
          ...
          <Grid sx={styles.cardIconSection}>
            ...
            <Checkbox {...label}
              ...
              onChange={() => checkDone(taskData.id)}
              defaultChecked={taskData.isDone}
            />
            <EditIcon onClick={() => updateTask(taskData.id)} />
            <DeleteIcon onClick={() => delModalOpen(taskData.id)} />
          </Grid>
        </CardContent>
      </Grid>
    </Card>
